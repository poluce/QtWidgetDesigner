...
struct DemoState
{
    ...
    int cursorMoveDurationMs = 700;
    int cursorMoveSteps = 20;
    int mousePressHoldMs = 140;
    int keyDelayMs = 90;
    int postActionPauseMs = 180;
    QSize cursorSize = QSize(26, 34);
    QPointer<QRubberBand> highlightBand;
    QPointer<QLabel> hintLabel;
    QPointer<QLabel> fakeCursorLabel;
    QPointer<QWidget> currentWindow;
    QPoint lastCursorTopLeft = QPoint(18, 18);
};
...
{
    const double multiplier = qMax(0.1, state().speedMultiplier);
