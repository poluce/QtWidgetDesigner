...
QPoint targetCursorTopLeft(QWidget* widget, const QPoint& localPoint)
{
    QWidget* window = ensureWindow(widget);
    if (window == nullptr) {
        return QPoint(kDefaultCursorX, kDefaultCursorY);
    }
...
class DemoActionObserver : public qtautotest::ActionObserver
{
public:
    ...
    void prepareForClick(QWidget* widget, const QPoint& localPoint) override
    {
        prepare(widget, localPoint, QStringLiteral("LLM 准备点击这里"));
    }

    void prepareForTyping(QWidget* widget) override
    {
        const QPoint center = widget != nullptr ? widget->rect().center() : QPoint();
        prepare(widget, center, QStringLiteral("LLM 准备在这里输入文字"));
    }

    void showClickPulse(QWidget* widget, const QPoint& localPoint) override
    {
        if (!state().enabled) {
            return;
        }
    ...
    int keyDelayMs() const override
    {
        return scaledDelay(state().keyDelayMs);
    }

    int mousePressHoldMs() const override
    {
        return scaledDelay(state().mousePressHoldMs);
    }
};

DemoActionObserver& observer()
{
    static DemoActionObserver instance;
    return instance;
}
...
void install()
{
    qtautotest::setActionObserver(&observer());
}

void uninstall()
{
    if (qtautotest::actionObserver() == &observer()) {
...
{
    state().speedMultiplier = qMax(kMinSpeedMultiplier, multiplier);
}
